...
{
    public class SendMailAwait : BaseSendMail
    {
        ...
        private const string c_RES_FMT = "  {4} [TID:{0:00}] Requested at {1} ... Response received at {2} [{3:c}]";
        ...
        public static async void SendEmail(string toAddress)
        {
            ...
            iConsole.WriteLine("  {2} [TID:{0:00}] Sending email at {1}", threadId, reqTime, reqLine);
            ...
            string fmtRes = String.IsNullOrEmpty(msgErr) ? c_RES_FMT : String.Concat(c_RES_FMT, " \n\t{5}");

            iConsole.WriteLine(String.Empty);
            iConsole.WriteLine(fmtRes, threadId, reqTime, resTime, (resTime - reqTime).ToString(@"dd\.hh\:mm\:ss"), resLine, msgErr);

            lock (m_Lock)
            {
                if (!Program.IsSummaryPrinted && Program.IsAllResponseCompleted)
                {
                    iConsole.WriteLine(String.Empty);
        ...
        public static void Client_SendCompleted(object sender, System.ComponentModel.AsyncCompletedEventArgs e)
        {
            ...
            if (Program.m_SendCompleted_Counter >= Program.toRecepients.Count)
                Program.IsAllResponseCompleted = true;
        }
    }
}
